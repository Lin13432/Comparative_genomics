...
# First of all, we need to delete all useless information before sequence, which means to delete 36 positions in each line
...
# We move every site from MSA to the list 'seq', so each element in the 'seq' represents one site on the sequence
for i in range(len(MSA)):
    for j in range(region_sites):
        seq[i, j] = list(MSA[i, 0])[j]

# We reshape the seq, now we have 13 regions like MSA result, each region has 72 rows (protein) and 50 columns(sites)
...
# Print all sites that has variability equal to 1, which means that amino acid conserved in all protwins
